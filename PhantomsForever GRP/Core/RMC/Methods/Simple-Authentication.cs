    ...
    public class Simple_Authentication
    {
        public class LoginWithTokenEx
        {
            ...
            public static LoginWithTokenEx Decode(string hex)
            {
                ...
                int tokensize = int.Parse(hex.Substring(0, 4).FromHex().Reverse().ToArray().ToHex().FromHexToDecimal().ToString());
                auth.Token = hex.Substring(4, tokensize);
                var data = new RVConnectionData();
                int surlsize = int.Parse(hex.Substring(4 + tokensize, 4).FromHex().Reverse().ToArray().ToHex().FromHexToDecimal().ToString());
                data.StationUrl = hex.Substring(8 + tokensize, surlsize);
                int spurlsize = int.Parse(hex.Substring(surlsize + 8 + tokensize, 8).FromHex().Reverse().ToArray().ToHex().FromHexToDecimal().ToString());
                if (spurlsize != 0)
                    data.SpecialProtocols = hex.Substring(surlsize + 16 + tokensize, spurlsize);
                int surlsize2 = int.Parse(hex.Substring(16 + surlsize + tokensize + spurlsize, 4).FromHex().Reverse().ToArray().ToHex().FromHexToDecimal().ToString());
                data.StationUrl2 = hex.Substring(20 + surlsize + tokensize + spurlsize, surlsize2);
                auth.RVConnectionData = data;
                int logindatalength = int.Parse(hex.Substring(20 + surlsize + tokensize + spurlsize + surlsize2, 4).FromHex().Reverse().ToArray().ToHex().FromHexToDecimal().ToString());
